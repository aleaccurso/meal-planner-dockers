...
```bash
make up          # check-env, ../deploy/deploy-latest.sh — reads each repo's latest tag, source .env.backend + .env.frontend, docker compose build && up -d
