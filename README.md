...
- **`../deploy/deploy-latest.sh`** (what `make up` calls) — read-only. Looks up whichever tag is already latest on each repo and builds from that, passing each resolved tag straight to `docker compose build --build-arg GIT_REF=... <service>` on the command line — `GIT_REF` is never an environment variable or something `docker-compose.yml` interpolates, so there's nothing for it to be silently unset. If a repo has never been tagged, `deploy-latest.sh` fails loudly telling you to run `tag-release.sh` first, rather than falling back to `main`. That said, a bare `docker compose build` that bypasses `deploy-latest.sh` entirely *will* silently fall back to the Dockerfiles' own `main` default — `deploy-latest.sh` is the only supported way to build.
